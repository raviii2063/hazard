...
router.post('/login', (req, res) => {
  ...
  db.get(query, [username.trim(), username.trim().toLowerCase()], (err, user) => {
    if (err) {
      return res.status(500).json({ error: 'Database error: ' + err.message });
    }
...
router.get('/me', authenticateToken, (req, res) => {
  db.get('SELECT id, username, full_name, email, role, created_at FROM users WHERE id = ?', [req.user.id], (err, user) => {
    if (err || !user) {
